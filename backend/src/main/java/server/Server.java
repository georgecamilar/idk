                    ...
                    switch (fields[0]) {
                        case "add":
                            if (fields.length - 1 < 3)
                                message.client.write(service.getAllTotalScores());
                            int idParticipant = Integer.parseInt(fields[1]);
                            int idProba = Integer.parseInt(fields[2]);
                            double grade = Double.parseDouble(fields[3]);
                            service.saveNota(new Nota(idParticipant, idProba, grade));
                            message.client.write(service.getAllTotalScores());
                            break;
                        case "login":
                            String username = fields[1];
                            String password = fields[2];
                            message.client.write(service.login(username, password));
                            break;
                        case "report":
                            Integer idUser = Integer.parseInt(fields[1]);
                            message.client.write(this.service.getRaportList(idUser));
                            break;
                        default:
